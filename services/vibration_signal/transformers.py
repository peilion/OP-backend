...
def get_hilbert_spectrum(data: ndarray):
...
def get_short_time_fournier_transform(data: ndarray):
...
def get_multi_scale_envelope_spectrum(data: ndarray, n_Fs: int, n_Ssta: float, n_Send: float, n_Sint: float):
        length = len(data)
...
def get_welch_spectrum_estimation(data: ndarray):
    freq, spec = signal.welch(data, 10000, scaling="spectrum")
    return {"freq": freq, "spec": spec}

def get_acceleration_to_velocity(data: ndarray):
...
def get_empirical_mode_decomposition(data: ndarray):
    decomposer = EMD(data)

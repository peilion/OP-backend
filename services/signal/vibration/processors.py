...
def fast_fournier_transform(signal: str) -> dict:
    signal = VibrationSignal(data=binary_deserializer(signal), fs=10000, compute_axis=False)
    signal.compute_spectrum(compute_axis=False)
    return {"spec": signal.spec, "vib": signal.data}


def hilbert(data: str) -> dict:
    signal = VibrationSignal(data=binary_deserializer(data), fs=10000, compute_axis=False)
    env = signal.to_envelope()
    env.compute_spectrum(compute_axis=False)
    return {"vib": signal.data,
            "env_vib": env.data,
            "env_fft": env.spec}
...
def acceleration_to_velocity(data: str) -> dict:
    ...
    return {"vel": vel.data, "acc": acc.data, "acc_spec": acc.spec, "vel_spec": vel.spec}

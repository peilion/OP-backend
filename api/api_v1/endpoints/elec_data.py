...
async def read_the_latest_electric_signal(mp_id: int):
...
async def read_electric_signal_by_id(mp_id: int, data_id: int):
    conn = Database(META_URL)
    res = await get_by_id(
        conn=conn,
        mp_id=mp_id,
...
@router.get(
    "/mp/{mp_id}/elec/{data_id}/analysis/",
    response_class=UJSONResponse
)
async def analyze_electric_signal(
    mp_id: int, data_id: int, method: AnalyzeRule = Query(None)
):
    conn = Database(META_URL)

...
async def update_motor_driven_threshold(diag_threshold: MotorDrivenEndPostSchema, conn: Database = Depends(get_db)):
    res = await update(conn=conn,mp_pattern='motor_driven',diag_threshold=diag_threshold.dict())
    if res == True:

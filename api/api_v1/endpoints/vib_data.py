...
async def read_the_latest_vibration_signal(mp_id: int):
    conn = Database(META_URL)
    res = await get_latest(
        conn=conn, mp_id=mp_id, orm_model=VibData, require_mp_type=0
    )
...
async def read_vibration_signal_by_id(mp_id: int, data_id: int):
    conn = Database(META_URL)
    res = await get_by_id(

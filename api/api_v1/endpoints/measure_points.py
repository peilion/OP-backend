...
@router.get("/", response_class=UJSONResponse, response_model=List[Optional[MeasurePointSchema]])
async def read_measure_points(
        skip: int = None,
        limit: int = None,
        station_id: int = Query(default=None,description='Filtering measure points with station\'s id' ),
        asset_id: int = Query(default=None,description='Filtering measure points with asset\'s id , **only one of the two filtering conditions shuold be given**' ),
):
    if station_id and asset_id:
        raise HTTPException(status_code=400, detail="Only one of station_id and asset_id should be given.")
    conn = Database(META_URL)
    item = await get_multi(conn=conn, skip=skip, limit=limit, station_id=station_id, asset_id=asset_id)
    return item
...
@router.get("/{id}/", response_class=UJSONResponse, response_model=Optional[MeasurePointSchema])

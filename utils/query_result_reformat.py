...
def format_double_grouped_result(res: list, fisrt_group_names: Optional[dict], second_group_names: dict):
    ...
    for item in res:
        first_name = fisrt_group_names[item[0]] if (fisrt_group_names is not None) else (item[0])
        second_name = second_group_names[item[1]]
        if res_dict.get(first_name) is None:
...
def format_map_grouped_result(res: list, fisrt_group_names: Optional[dict], second_group_names: dict):
    res_dict = {}

...
for station in stations:
    assets = session.query(Asset).filter(Asset.asset_type == 0, Asset.station_id == station.id).all()
    inner_id = -1
    for asset in assets:
        if asset.mp_configuration == 1:
            for name in [["电机非驱动端Y向", 'vertical', 'motor_non_driven'],
                         ["电机驱动端Y向", 'vertical', 'motor_driven'],
                         ["泵非驱动端Y向", 'vertical', 'pump_non_driven'],
                         ["泵驱动端Y向", 'vertical', 'pump_driven']]:
                inner_id = inner_id + 1
                session.add(MeasurePoint(
                    type=0,
                    name=name[0],
                    sample_interval=300,
                    sample_freq=10000,

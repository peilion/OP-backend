...
query = session.query(MeasurePoint.station_id, MeasurePoint.inner_station_id) \
...
base_data_model = VibData.model(station_id=mps_info[0]['station_id'], inner_id=mps_info[0]['inner_station_id'])
base_data_query = session.query(base_data_model.time, base_data_model.rms).order_by(base_data_model.time.desc())

...
class Tree(object):
    ...
    def __get_iter(self, nid, level, filter_, key, reverse, dt, is_last):
        ...
        if level == self.ROOT:
            ...
            leading = "".join(
                map(lambda x: dt_vline + " " * 3 if not x else " " * 4, is_last[0:-1])
            )
    ...
    def to_dict(self, nid=None, key=None, sort=True, reverse=False, with_data=False):
        ...
        ntag = self[nid].tag
        tree_dict = {"children": []}
        # tree_dict.append({"children": []})
        if with_data:
            if not (self[ntag].data):
                pass
            else:
                tree_dict = {**tree_dict, **self[nid].data}
    ...
    #             pass
    #         else:
    #             tree_dict[ntag] = { **tree_dict[ntag], **self[nid].data}
    #         # tree_dict[ntag]["data"] = self[nid].data
    #
    #     if self[nid].expanded:
    #         queue = [self[i] for i in self[nid].fpointer]

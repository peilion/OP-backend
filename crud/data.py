...
@con_warpper
async def get_latest(conn: Database, orm_model, shard_id: int, session: Session = session_make(engine=None), ):
...
@con_warpper
async def get_by_id(conn: Database, orm_model, shard_id: int, data_id: int,
...
@con_warpper
async def get_multi(conn: Database, orm_model, shard_id: int, time_before: datetime, time_after: datetime,
                    session: Session = session_make(engine=None)):
    model = orm_model.model(point_id=shard_id)
    query = session. \

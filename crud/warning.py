...
async def get(conn: Database, id: int, session: Session = session_make(engine=meta_engine)):
    ...
    if not res.is_read:
        session.query(WarningLog).filter(WarningLog.id ==
                                         id).update({WarningLog.is_read: True})
        session.commit()
        session.close()
    return res
...
async def get_warning_stat_by_station(conn: Database, session: Session = session_make(engine=None)):
    query = session. query(
        ...
        Station.name,
        func.count(
            WarningLog.asset_id).label('cnt')).select_from(WarningLog). join(
        Asset,
    ). join(
        Station,
        Asset.station_id == Station.id). group_by(
        Asset.station_id)
...
@con_warpper
async def get_warning_stat_by_branch_company(conn: Database, session: Session = session_make(engine=None)):
    query = session. query(
        BranchCompany.name,
        func.count(
            WarningLog.asset_id).label('cnt')).select_from(WarningLog). join(
        ...
        Station,
        Asset.station_id == Station.id). join(
        BranchCompany,
        Station.bc_id == BranchCompany.id) . group_by(
        Asset.station_id)
...
@con_warpper
async def get_warning_stat_by_region_company(conn: Database, session: Session = session_make(engine=None)):
    query = session. query(
        RegionCompany.name,
        func.count(
            WarningLog.asset_id).label('cnt')).select_from(WarningLog). join(
        ...
        Asset.station_id == Station.id). join(
        RegionCompany,
        Station.rc_id == RegionCompany.id) . group_by(
        Asset.station_id)
...
@con_warpper
async def get_warning_stat_by_isreadable(conn: Database, session: Session = session_make(engine=None)):
    query = session.query(WarningLog.is_read, func.count('*').label('cnt')). \

...
async def get_count_by_statu(conn: Database):
    query = 'SELECT statu, COUNT(*) as cnt ' \
            'FROM `asset` ' \
            'GROUP BY statu'
...
async def get_count_by_province(conn: Database):
    query = 'SELECT s.location, COUNT(*) as cnt ' \
            'FROM asset a ' \
...
async def get_count_by_oil_type(conn: Database):
    query = 'SELECT pu.oil_type, COUNT(*) as cnt ' \
            'FROM asset a ' \
            'JOIN pump_unit pu on a.id = pu.asset_id ' \
            'where a.asset_type=0 ' \
...
async def get_count_by_region_company(conn: Database):
    query = 'SELECT rc.name, COUNT(*) as cnt ' \
            'FROM asset a ' \
...
async def get_count_by_branch_company(conn: Database):
    query = 'SELECT bc.name, COUNT(*) as cnt ' \
            'FROM asset a ' \
...
async def get_count_by_station(conn: Database):
    query = 'SELECT station_id, COUNT(*) as cnt ' \
            'FROM `asset` ' \
            'GROUP BY station_id'
    res = await conn.fetch_all(query)
...
async def get_count_by_asset_type(conn: Database):
    query = 'SELECT asset_type, COUNT(*) as cnt ' \
            'FROM `asset` ' \
            'GROUP BY asset_type'
    res = await conn.fetch_all(query)
...
async def get_count_by_isdomestic(conn: Database):
    query = 'SELECT pu.is_domestic, COUNT(*) as cnt ' \
            'FROM asset a ' \
            'JOIN pump_unit pu on a.id = pu.asset_id ' \
            'where a.asset_type=0 ' \
            'group by pu.is_domestic'
    res = await conn.fetch_all(query)
    res_dic = format_single_grouped_result(res=res, group_names={0: 'Improted', 1: 'Domestic'})
    return res_dic


@con_warpper
async def get_count_by_manufacturer(conn: Database):
    query = 'SELECT m.name, COUNT(*) as cnt ' \
            'FROM `asset` ' \
...
async def get_statu_count_by_station(conn: Database):
    query = 'SELECT station.NAME, statu, COUNT( * ),station.longitude, station.latitude AS cnt ' \
            'FROM `asset` ' \
            'JOIN station ON asset.station_id = station.id  ' \
            'GROUP BY statu, station_id  ' \
            'ORDER BY statu'
    res = await conn.fetch_all(query)
    formatted_res = format_map_grouped_result(res, fisrt_group_names=None, second_group_names=Asset.STATUS)
    return formatted_res


@con_warpper
async def get_statu_count_by_type(conn: Database):
    query = 'SELECT asset_type,statu, COUNT(*) as cnt ' \
            'FROM `asset` ' \
            'GROUP BY statu,asset_type ' \
            'ORDER BY statu'
    res = await conn.fetch_all(query)
    formatted_res = format_double_grouped_result(res, fisrt_group_names=Asset.TYPES, second_group_names=Asset.STATUS)
    return formatted_res


@con_warpper
async def get_statu_count_by_province(conn: Database):
    query = 'SELECT s.location,a.statu, COUNT(*) as cnt ' \
            'FROM asset a ' \
            'JOIN station s on a.station_id = s.id ' \
    ...
    res = await conn.fetch_all(query)
    formatted_res = format_double_grouped_result(res, fisrt_group_names=None, second_group_names=Asset.STATUS)
...
async def get_statu_count_by_pipeline(conn: Database):
    query = 'SELECT p.name, a.statu, COUNT(*) as cnt ' \
            'FROM asset a ' \
            'JOIN pump_unit pu on a.id = pu.asset_id ' \
            'JOIN pipeline p on pu.pipeline_id = p.id ' \
            'where a.asset_type=0 ' \
...
async def get_statu_count_by_oiltype(conn: Database):
    query = 'SELECT pu.oil_type,a.statu, COUNT(*) as cnt ' \
            'FROM asset a ' \
            'JOIN pump_unit pu on a.id = pu.asset_id ' \
            'where a.asset_type=0 ' \
    ...
    res = await conn.fetch_all(query)
    formatted_res = format_double_grouped_result(res, fisrt_group_names=PumpUnit.OIL_TYPES,
...
async def get_statu_count_by_region_company(conn: Database):
    query = 'SELECT rc.name,a.statu, COUNT(*) as cnt ' \
            'FROM asset a ' \
            'JOIN station s on a.station_id = s.id ' \
            ...
            'order by a.statu'
    res = await conn.fetch_all(query)
    formatted_res = format_double_grouped_result(res, fisrt_group_names=None,
                                                 second_group_names=Asset.STATUS)
    return formatted_res
...
async def get_statu_count_by_branch_company(conn: Database):
    query = 'SELECT bc.name,a.statu, COUNT(*) as cnt ' \
            'FROM asset a ' \
            'JOIN station s on a.station_id = s.id ' \
...
async def get_statu_count_by_isdomestic(conn: Database):
    query = 'SELECT pu.is_domestic,a.statu, COUNT(*) as cnt ' \
            'FROM asset a ' \
            'JOIN pump_unit pu on a.id = pu.asset_id ' \
            'where a.asset_type=0 ' \
            'group by a.statu, pu.is_domestic'
    res = await conn.fetch_all(query)
    formatted_res = format_double_grouped_result(res, fisrt_group_names={0: 'Imported', 1: 'Domestic'},
                                                 second_group_names=Asset.STATUS)
    return formatted_res
...
async def get_statu_count_by_manufacturer(conn: Database):
    query = 'SELECT m.name,a.statu, COUNT(*) as cnt ' \
            'FROM `asset` a ' \
...
async def get_type_count_by_station(conn: Database):
    ...
    res = await conn.fetch_all(query)
    formatted_res = format_double_grouped_result(res, fisrt_group_names=None,
                                                 second_group_names=Asset.TYPES)
    res_list = []

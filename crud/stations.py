...
async def get_tree(conn: Database, session: Session = session_make(engine=None)):
    query1 = session. \
        query(Station.id, Station.name, Station.bc_id)
    query2 = session. \
        query(BranchCompany.id, BranchCompany.name, BranchCompany.rc_id)
    query3 = session. \
        query(RegionCompany.id, RegionCompany.name)
    query4 = session. \
        query(Asset.id, Asset.name, Asset.station_id).filter(Asset.asset_level == 0)
    ...
    random.choice(color)
    for item in assets:
        tree.create_node(data={'name': item['name'], 'parent_st_id': item['station_id'], 'value': 1, 'itemStyle': {'color':random.choice(color)}},
                         identifier='asset' + str(item['id']),
                         parent='root')

    for item in stations:
        tree.create_node(data={'name': item['name'], 'parent_bc_id': item['bc_id'],  'itemStyle': {'color':random.choice(color)}},
                         identifier='st' + str(item['id']),
                         parent='root')

    for item in bcs:
        tree.create_node(data={'name': item['name'], 'parent_rc_id': item['rc_id'],  'itemStyle': {'color':random.choice(color)}},
                         identifier='bc' + str(item['id']),
                         parent='root')

    for item in rcs:
        tree.create_node(data={'name': item['name'], 'itemStyle': {'color':random.choice(color)}},
...
@con_warpper
async def get_bc(conn: Database, skip: int, limit: int, session: Session = session_make(engine=None)):
    query = session. \
        query(BranchCompany). \
...
@con_warpper
async def get_rc(conn: Database, skip: int, limit: int, session: Session = session_make(engine=None)):
    query = session. \
        query(RegionCompany). \

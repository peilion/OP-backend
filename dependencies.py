...
def get_mp_mapper():
    if len(measure_point_regsiter) == 0:
        ...
        res = session.query(MeasurePoint.id, MeasurePoint.station_id, MeasurePoint.id_inner_station).all()
        for row in res:
            measure_point_regsiter[row.id] = {'station_id': row.station_id, 'inner_id': row.id_inner_station}
        session.close()

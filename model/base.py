...
class SubSampledArray(list):
    ...
    def validate(cls, v:ndarray):
        axis = np.linspace(0, v.size, int(v.size / TIME_DOMAIN_SUB_SAMPLED_RATIO), endpoint=False)
        return [round(float(item), TIME_DOMAIN_DECIMAL) for item in np.take(v, axis.astype(np.int))]

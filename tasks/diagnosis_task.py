...
position_type_mapper = {
    'motor_non_driven': (2, '非', motor_non_driven_end_diagnosis),  # asset_type, bearing position , and diagnosis method
...
def fetch_assets(session):
    assets = session.query(Asset.id, Asset.name, Asset.parent_id, Asset.asset_type).all()
    tree = Tree()
    tree.create_node(tag="root", identifier="root")
    for item in assets:
        tree.create_node(
            data=item._asdict(),
            identifier=item.id, parent="root")
...
def fetch_mps(session):
    mps = session.query(MeasurePoint.id, MeasurePoint.name, MeasurePoint.station_id, MeasurePoint.inner_station_id,
                        MeasurePoint.asset_id, MeasurePoint.last_diag_id, MeasurePoint.position,
                        MeasurePoint.direction).filter(MeasurePoint.type == 0, MeasurePoint.position != 'pipeline').all()
    return mps
...
def fetch_data(session, mp: Dict):
    ...
    signals = session.query(data_model.id, data_model.time,data_model.ima.label('vib')).filter(
        data_model.id > last_diag_id).limit(10).all()
    return signals
    ...
    for equip in asset['children']:
        if equip['asset_type'] == position_type_mapper[mp['position'].name][0]:
            for bearing in equip['children']:
                if bearing['name'].startswith(position_type_mapper[mp['position'].name][1]):
                    bearing_id = bearing['id']

    bearing_info = session.query(Bearing.bpfi, Bearing.bpfo, Bearing.bsf, Bearing.ftf). \
    ...
    for (asset_id, asset) in assets.items():  # 遍历所有设备
        if 'mps' in asset:  # 跳过未添加测点的设备
            for (mp_name, mp) in asset['mps'].items():  # 遍历某设备的所有测点
                ...
                # 获取轴承信息
                bearing_id, bearing_info = fetch_bearing_info(session, asset=asset, mp=mp)

                if (bearing_id is not None) & (len(bearing_info) != 0):
                    th = session.query(Threshold.diag_threshold).filter(
                        Threshold.mp_pattern == mp['position'].name).one()

                    diag_res_insert_value = []
                    for index, signal in enumerate(signal_list):
                        diag_method = position_type_mapper[mp['position'].name][2]
                        diag_res, indicators = diag_method(data=signal.vib, fs=10000, R=2800,
                            ...
                            if diag_res_sum != 0:
                                diag_res_insert_value.append(
                                    WarningLog(
                                        ...
                                        severity=int(diag_res_sum - 1) if diag_res_sum < 3 else 2,
                                        asset_id=asset_id,
                                        mp_id=mp['id'],
                                        cr_time=signal.time,
                                        is_read=False,
                                        data_id=signal.id, **indicators)
                                )
                                session.add_all(diag_res_insert_value)
                            if index == len(signal_list) - 1:
                                session.query(MeasurePoint).filter(MeasurePoint.id == mp['id']).update(
                                    {'id': mp['id'],
                                     'statu': int(diag_res_sum) if diag_res_sum < 4 else 3,
